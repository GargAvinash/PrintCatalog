...
- **GitHub Issue Integration:** To automatically close an issue upon merging to the `main` branch, the commit message MUST include a closing keyword (e.g., `fixes`, `closes`, `resolves`) followed immediately by the issue number. Example: `feat: add boundary validation (fixes #1)`. Simply appending the issue number without a keyword will only link the commit to the issue without closing it.
